...
public class Sprite3D {
    ...
    public void draw(Texture2dProgram program, float[] projectionMatrix, float[] texMatrix, int sourceWidth, int sourceHeight, int targetWidth, int targetHeight) {
        Matrix.multiplyMM(scratchMatrix, 0, projectionMatrix, 0, modelMatrix, 0);
        program.draw(scratchMatrix, drawable.getVertexArray(), 0,
                drawable.getVertexCount(), drawable.getCoordsPerVertex(),
                drawable.getVertexStride(), texMatrix, drawable.getTexCoordArray(),
                textureId, drawable.getTexCoordStride(), sourceWidth, sourceHeight, targetWidth, targetHeight);
    }
